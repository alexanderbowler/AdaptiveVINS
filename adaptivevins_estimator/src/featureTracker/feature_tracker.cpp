...
void FeatureTracker::setMask()
{
    ...
    for (auto &it : cnt_pts_id)
    {
        if (mask.at<uchar>(it.second.first) == 255)
        {
            ...
            // Mark a MIN_DIST-radius exclusion zone around this point
...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    */
    ...
    if (prev_pts.size() > 0)
    {
        ...
        if(hasPrediction)
        {
            cur_pts = predict_pts; // seed optical flow with IMU-predicted positions
            // cv::OPTFLOW_USE_INITIAL_FLOW: use cur_pts as initial estimate instead of copying from prev_pts
            // TermCriteria: stop after 30 iterations or 0.01px error
            ...
            // If too few points tracked, retry without initial flow
            if (succ_num < 10)
        ...
        // Reverse check: track back from current to previous and reject inconsistent matches
        if(FLOW_BACK)
        {
            ...
            for(size_t i = 0; i < status.size(); i++)
            {
                // Accept if both forward and backward tracking succeeded and round-trip error <= 0.5px
    ...
    if (1)
    {
        ...
        TicToc t_m;
        // Re-sort by track count and mark MIN_DIST exclusion zones
        setMask();
        ROS_DEBUG("set mask costs %fms", t_m.toc());

        ROS_DEBUG("detect feature begins");
        TicToc t_t;
        int n_max_cnt = MAX_CNT - static_cast<int>(cur_pts.size()); // how many new features we still need
        if (n_max_cnt > 0)
        {
            if(mask.empty())
            ...
            // Detect new features in unoccupied regions up to MAX_CNT total
    ...
    // Compute feature velocities on normalized plane (cur_un_pts_map is updated inside)
    pts_velocity = ptsVelocity(ids, cur_un_pts, cur_un_pts_map, prev_un_pts_map);

    // Stereo: track current left features into the right image (not temporal tracking)
    if(!_img1.empty() && stereo_cam)
    {
        ...
        if(!cur_pts.empty())
        {
            ...
            vector<float> err;
            // cur left ---- cur right
            cv::calcOpticalFlowPyrLK(cur_img, rightImg, cur_pts, cur_right_pts, status, err, cv::Size(21, 21), 3); // track left features into right image
            ...
            // Remove failed stereo matches
            ids_right = ids;
            reduceVector(cur_right_pts, status);
            reduceVector(ids_right, status);
            cur_un_right_pts = undistortedPts(cur_right_pts, m_camera[1]); // undistort + back-project
            right_pts_velocity = ptsVelocity(ids_right, cur_un_right_pts, cur_un_right_pts_map, prev_un_right_pts_map); // velocity on normalized plane
        }
...
vector<cv::Point2f> FeatureTracker::undistortedPts(vector<cv::Point2f> &pts, camodocal::CameraPtr cam)
{
    ...
    for (unsigned int i = 0; i < pts.size(); i++)
    {
        ...
        un_pts.push_back(cv::Point2f(b.x() / b.z(), b.y() / b.z())); // b.z() == 1.0 for pinhole
    }
    return un_pts;
}
    ...
    if (!prev_id_pts.empty())
    {
        ...
        for (unsigned int i = 0; i < pts.size(); i++)
        {
            ...
            if (it != prev_id_pts.end()) // matching feature found in previous frame
            {
                ...
                pts_velocity.push_back(cv::Point2f(0, 0)); // no match: velocity = 0
        }
    }
